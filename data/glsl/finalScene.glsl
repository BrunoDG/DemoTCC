...
vec2 field( vec3 position )
{
    ...
    vec2 to2 = torus( position, vec2( 5.0,.15), zero, vec4( 0., 2., 1., PI * .8 + time * .2) );
    ...
    //cone
    //vec2 co = cone(position, 1., zero, quat);

    //cylinder
    vec2 cy = cylinder(position, 4., 2., zero, quat);

    //composition
    return smin( sce, smin( to0, smin( to1, smin( to2, subtract( cy, subtract(sre, rb) ), pnoise), pnoise ), pnoise ), pnoise);
